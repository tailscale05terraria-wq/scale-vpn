...
class _MainVpnScreenState extends State<MainVpnScreen> with TickerProviderStateMixin {
  ...
  // ВАШИ ССЫЛКИ + ЗЕРКАЛА ДЛЯ НАДЕЖНОСТИ ОТКРЫТИЯ В РФ
  final List<String> primarySources = [
    ...
    // Зеркала jsdelivr (открываются всегда, если оператор заблокировал raw.githubusercontent.com)
  ...
  void _togglePin(ServerNode item) {
    setState(() {
      ...
      if (item.isPinned) {
        ...
        _showToast("Узел закреплен", isSuccess: true);
      } else {
  ...
  List<String> _extractConfigs(String rawData) {
    ...
    for (var line in lines) {
      line = line.trim();
      // Фильтруем только поддерживаемые протоколы ядра
  ...
  // ЗАГРУЗКА И ПРОВЕРКА ПАРТИИ ИЗ 15 КЛЮЧЕЙ (БЕЗ ПОВТОРНЫХ КРУГОВ)

  Future<void> _searchGitHubForKeys() async {
    if (isSearchingGitHub) return;
    setState(() {
      isSearchingGitHub = true;
      searchStatusText = "Загрузка списков igareck...";
    });
    ...
    try {
      ...
      // 1. Параллельный опрос всех ссылок сразу с мобильным User-Agent
      final futures = primarySources.map((url) async {
        try {
      ...
      if (pool.isEmpty) {
        if (mounted) _showToast("Не удалось открыть ссылки. Проверьте интернет", isSuccess: false);
        return;
      }

      // 2. Случайно перемешиваем и берем РОВНО 15 КЛЮЧЕЙ
      final allList = pool.toList()..shuffle(_random);
      final List<String> batch15 = allList.take(15).toList();

      // Оставляем закрепленные пользователем ключи
      final List<ServerNode> workingNodes = autoServers.where((s) => s.isPinned).toList();

      // Подготавливаем эти 15 ключей к замеру
      final List<ServerNode> testBatch = [];
      for (int i = 0; i < batch15.length; i++) {
        final cfg = batch15[i];
        String title = "Узел ${i + 1}";
        if (cfg.contains('#')) {
          try {
            title = Uri.decodeComponent(cfg.split('#').last).trim();
          } catch (_) {
            title = cfg.split('#').last.trim();
          }
        }
        testBatch.add(ServerNode(
          name: title.isNotEmpty ? title : "Узел ${i + 1}",
          rawConfig: cfg,
          ping: -2,
          isPinned: pinnedConfigs.contains(cfg),
        ));
      }

      if (mounted) {
        setState(() {
          autoServers = [...workingNodes, ...testBatch];
          selectedIndex = 0;
        });
      }

      // 3. БЫСТРАЯ ОДНОКРАТНАЯ ПРОВЕРКА ЭТИХ 15 КЛЮЧЕЙ (по очереди)
      int checkedCount = 0;
      for (final node in testBatch) {
        if (!mounted) break;
        checkedCount++;
        setState(() {
          searchStatusText = "Проверка узла $checkedCount из ${testBatch.length}...";
        });

        final delay = await _testRealDelayInternal(node.rawConfig);
        if (mounted) {
          setState(() {
            node.ping = delay;
          });
      ...
      // 4. ОТСЕИВАЕМ НЕРАБОЧИЕ И НЕ НАЧИНАЕМ НОВЫЙ ПОИСК
      setState(() {
        autoServers.removeWhere((node) => !node.isPinned && node.ping <= 0);
        _sortNodes(autoServers);
        if (selectedIndex >= autoServers.length) selectedIndex = 0;
      });

      if (autoServers.isEmpty) {
        _showToast("В этой пачке нет рабочих. Нажмите «Синхронизация» еще раз", isSuccess: false);
      } else {
        _showToast("Готово! Найдено рабочих узлов: ${autoServers.length}", isSuccess: true);
      }
    } catch (_) {
      if (mounted) _showToast("Ошибка при чтении ссылок", isSuccess: false);
    } finally {
      if (mounted) {
        setState(() {
  ...
  // Быстрый честный тест через Xray (таймаут 1.8 с)
  Future<int> _testRealDelayInternal(String rawConfig) async {
    ...
    try {
      final delay = await flutterV2ray
          ...
          .timeout(const Duration(milliseconds: 1800), onTimeout: () => -1);
      return delay;
    } catch (_) {
      return -1;
    }
    ...
    try {
      ...
      if (permissionGranted) {
        ...
        if (mounted) {
          ...
          _gearController.stop();
          _showToast("Разрешение VPN отклонено", isSuccess: false);
        }
      }
    } catch (_) {
      if (mounted) {
        setState(() => isConnecting = false);
        _gearController.stop();
        _showToast("Ошибка конфигурации Xray", isSuccess: false);
      }
    }
  }
  ...
  void _handleToggle() async {
    ...
    if (activeList.isEmpty) {
      _showToast("Список узлов пуст. Загрузка...", isSuccess: false);
        ...
        actions: [
          ...
          ElevatedButton(
            ...
            onPressed: () {
              ...
              _showToast("Ключ добавлен", isSuccess: true);
            },
        ...
        content: const SingleChildScrollView(
          child: Column(
            ...
            children: [
              ...
              SizedBox(height: 8),
              Text(
                "1. Прямой доступ к репозиториям igareck и зеркалам.\n"
                "2. Из всех ссылок случайным образом отбирается порция из 15 ключей.\n"
                "3. Каждый узел из 15 тестируется ядром Xray.\n"
                "4. Неработающие узлы сразу удаляются, а рабочие остаются в списке без долгих повторных ожиданий.",
        ...
        children: [
          Row(
            ...
            children: [
              Row(
                children: [
                  _buildTabButton("Узлы igareck", 0),
                  const SizedBox(width: 8),
  ...
  Widget _buildEmptyState() {
    ...
    return Center(
      child: TextButton.icon(
        ...
        label: const Text("Загрузить 15 узлов из ссылок",
            style: TextStyle(color: Palette.brass, fontSize: 12)),
